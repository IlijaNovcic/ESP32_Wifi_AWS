...
/*
 * Wifi applciation event group handler and status bits
 */
static EventGroupHandle_t wifi_app_event_group;
const uint8_t WIFI_APP_CONNECTING_USING_SAVED_CREDS_BIT	  = BIT0;
const uint8_t WIFI_APP_CONNECTING_FROM_HTTP_SERVER_BIT 		= BIT1;
const uint8_t WIFI_APP_USER_REQUESTED_STA_DISCONNECT_BIT 	= BIT2;
const uint8_t WIFI_APP_STA_CONNECTED_GOT_IP_BIT						= BIT3;
...
static void wifi_app_soft_ap_config(void)
{
	...
	memset(&ap_ip_info, 0x00U, sizeof(ap_ip_info));
...
/*
 * COnnects the ESP to an external AP using the updated station configuration
 */
		...
		if (xQueueReceive(wifi_app_queue_handle, &msg, portMAX_DELAY))
		{
			switch (msg.msgID)
			{
				...
				case WIFI_APP_MSG_STA_CONNECTED_GOT_IP:
					...
					http_server_monitor_send_message(HTTP_MSG_WIFI_CONNECT_SUCCES);

					eventBits = xEventGroupGetBits(wifi_app_event_group);

					if(eventBits & WIFI_APP_CONNECTING_USING_SAVED_CREDS_BIT) ///> Save STA creds only if connecting from the http server and not loaded from NVS
					{
						xEventGroupClearBits(wifi_app_event_group, WIFI_APP_CONNECTING_USING_SAVED_CREDS_BIT); ///> Clear the bits, in case we want to disconnect
					}
					else
					{
					...
					{
						xEventGroupClearBits(wifi_app_event_group, WIFI_APP_CONNECTING_FROM_HTTP_SERVER_BIT); ///> Clear the bits, in case we want to disconnect
					}
...
void wifi_app_start(void)
{
	...
	//ALocate memory for the wifi configuration
	wifi_config = (wifi_config_t*)malloc(sizeof(wifi_config_t));
	memset(wifi_config, 0x00, sizeof(wifi_config_t));

	// Create message queue
	wifi_app_queue_handle = xQueueCreate(3U, sizeof(wifi_app_queue_message_t));
