...
esp_err_t app_nvs_save_sta_creds(void)
{
	...
	if(wifi_sta_config != (void*)0u)
	{
		...
		if(esp_err != ESP_OK)
		{
			printf("app_nvs_save_sta_creds: Error (%s) Commit credentials to NVS!\n", esp_err_to_name(esp_err));
			return esp_err;
		}
...
bool app_nvs_load_sta_creds(void)
{
	...
	if(nvs_open(app_nvs_sta_creds_namespace, NVS_READONLY, &handle) == ESP_OK)
	{
		...
		memset(wifi_config_buffer, 0x0u, sizeof(wifi_config_size));
...
esp_err_t app_nvs_clear_sta_creds(void)
{
	...
	if(esp_err != ESP_OK)
	{
		printf("app_nvs_clear_sta_creds: (%s) opening NVS handle!\n", esp_err_to_name(esp_err));
		return esp_err;
	}
	...
	if(esp_err != ESP_OK)
	{
		printf("app_nvs_clear_sta_creds: (%s) erasing credentials!\n", esp_err_to_name(esp_err));
		return esp_err;
	}
	...
	if(esp_err != ESP_OK)
	{
		printf("app_nvs_clear_sta_creds: (%s) NVS commit!\n", esp_err_to_name(esp_err));
		return esp_err;
	}
	nvs_close(handle);
